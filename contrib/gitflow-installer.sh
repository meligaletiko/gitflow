...
if [ -z "$REPO_HOME" ] ; then
	REPO_HOME="http://github.com/portellaa/gitflow.git"
fi

EXEC_FILES="git-flow"
SCRIPT_FILES="git-flow-init git-flow-bugfix git-flow-feature git-flow-hotfix git-flow-release git-flow-sprint git-flow-support git-flow-version gitflow-common gitflow-shFlags"
